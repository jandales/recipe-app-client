...
const getRecipes = async(page = 0) => {        
    await store.dispatch('getSavedRecipes', {id : route.params.id, page : page}) 
    recipes.value = [...recipes.value, ...userSavedRecipes.value] 
    lastEntryCount.value = userSavedRecipes.value;     
}
    
const userSavedRecipes = computed(() => { return store.getters.savedRecipes });
...
const isAuthUser = inject('isAuthUser');
...
<template>  
     <div v-if="!isAuthUser(user_id)"  class=" w-full flex flex-col justify-center items-center mt-8">
       ...
       <h1 class="text-lg block text-center text-gray-500 mt-4">Private</h1>
    </div>
    ...
    <div  v-if="recipes && recipes.length == 0" class=" w-full flex flex-col justify-center items-center mt-8">
        ...
        <h1 class="text-lg block text-center text-gray-500">Add Recipe in your Collection</h1>
    </div>
